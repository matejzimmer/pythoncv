...
for i in range(current_year, current_year + 5):
    easter_date = datetime(i, 4, 1) + relativedelta(weekday=6)
    print(f"Velikonoční neděle v roce {i}: {easter_date.strftime('%d.%m.%Y')}")
...
current_year = datetime.now().year
while True:
    christmas_date = datetime(current_year, 12, 24)
    if christmas_date.weekday() == 6:  # Neděle (0=pondělí, 1=úterý, ..., 6=neděle)
